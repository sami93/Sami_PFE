...
def import_dataset():
    ...
    global Matricule
    global test
    column_names = ['Matricule', 'Civilite', 'EXPERIENCE_AVANT_SOFRECOM', 'EXPERIENCE_SOFRECOM', 'EXPERIENCE_Totale',
                    'Seniorite', 'Ecole', 'Niveau_Academique', 'Dernier_Employeur', 'Poste', 'Pole', 'Manager', 'Age',
                    'SITUATION_FAMILIALE', 'DEM']
...
def Split():
    ...
    x_train, x_test, y_train, y_test = train_test_split(training_set[FEATURES], prediction_set, train_size=0.8,
                                                        test_size=0.2,
                                                        random_state=42)
...
@app.route("/PredictionPerEmployee", methods=["GET"])
